...
const fetchDogs = (breed) => {
    return axios.get(`https://dog.ceo/api/breed/${breed}/images`).then(res => res).catch(err => console.error('noooo'));
}
...
class App extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        if (prevState.doggos !== this.state.doggos) {
            ...
            if (this.state.breed === 'chihauhua') {
                fetchDogs('husky').then(res => {
                    this.setState({doggos: res.date.message, breed: 'husky'})
                })
            }
        }
